...
st.caption("Data sourced from [Yahoo Finance](https://finance.yahoo.com/). Visit their website to find valid ticker symbols.")
# Two rows, each with two columns for inputs
row1 = st.columns(2)

ticker1 = row1[0].text_input("Enter the first ticker symbol:", "^NDX")
ticker2 = row1[1].text_input("Enter the second ticker symbol:", "BTC-USD")
...
try:
    ...
    data1 = yf.download(ticker1, start=start_date, end=end_date, auto_adjust=False)['Close']
    data2 = yf.download(ticker2, start=start_date, end=end_date, auto_adjust=False)['Close']
    ...
    window_size = st.slider("Window Size", min_value=1, max_value=365*4, value=150, step=10)
